 ...
 */
public class DatePickerPlugin /* extends Plugin */ implements JSONBusHandler
{
  public DatePickerPlugin(JSONBusActivity activity) {
    myactivity = activity;
  }
  ...
  public void jsonmessage(String s) {
    ...
    try {
      //arr = new JSONArray("[{mode: 'date'}]");
      arr = new JSONArray("[{mode: 'date', date: '12/12/2012/12/12'}]");
    } catch (JSONException ex) {
      ex.printStackTrace();
      return;
    }

    this.execute(ACTION_DATE, arr, "123");
  }

  private JSONBusActivity myactivity;
	 ...
	 */
	//@Override
	public /* PluginResult */ void execute(final String action, final JSONArray data, final String callBackId) {
...
**/
	...
	public synchronized void show(final JSONArray data, final String callBackId) {
		...
		final JSONBusActivity currentCtx = myactivity;
	...
	if (wasfired) return;
	wasfired = true;
	Log.d(pluginName, "sending return date" + returnDate);
			//datePickerPlugin.success(new PluginResult(PluginResult.Status.OK, returnDate), callBackId);
			myactivity.sendJavascript("window.datecb(\"{date:'" + returnDate + "'}\")");
		}
	}
